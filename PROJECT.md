...
- 60~87일(후기1) 자동 생성 규칙: 돼지고기는 아직 미도입(세정 요청, 소고기·닭고기만 안전). 채소 목록에서 청경채는 빼고 양파로 대체. 신규 재료(계란 포함)는 서로 3일 이상 간격 + 평일에만 도입 — 당근(7/13)→흰살생선(7/16, 고기)→계란노른자 알러지테스트(7/20)→새송이버섯(7/23)→토마토(7/27)→무(7/30)→검은콩(베이스, 8/3)→퀴노아(베이스, 8/6). 신규 재료는 도입일에 1끼(아침)에만 넣고 2끼는 대체, 다음날부터 양쪽 끼니에 정식 합류. 고기·채소 모두 매일 바꾸지 않고 1주일 단위로 고정. 검은콩은 현미 대체(격일 교대)라 총량 불변, 퀴노아는 대체가 아니라 넷째 베이스로 추가라서 8/7부터 끼니당 15g씩 늘어남(1끼 200g·2끼 190g, 세정에게 안내 완료). 끼니 구성은 베이스 75g(쌀30·오트밀30·현미/검은콩15, +퀴노아15) + 채소 6종×15g(90g) + 고기(1끼20g·2끼10g)
...
- 2026-07-09: 7/10부터 비어있던 메뉴 채움. 7/10~7/12는 7/9와 동일 메뉴로 복제, 7/13~8/9(60~87일)는 신규 재료 3일 주기 도입 + 7/17 계란 알러지 테스트 스케줄을 반영한 자동 생성 로직 도입.
